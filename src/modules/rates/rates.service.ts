...
import {
  findValidCachedRates,
  upsertRates,
  type CachedRateRow,
} from "./rates.repository.js";

type RatesMap = Partial<
...
export async function getRates(
  ...
  try {
    ...
    if (
      ...
      return {
        ...
        updatedAt:
          cachedRates[0].fetched_at.toISOString(),
        expiresAt:
          cachedRates[0].expires_at.toISOString(),
  ...
  return {
    ...
    updatedAt:
      providerResult.fetchedAt.toISOString(),
    expiresAt:
...
export async function getRatePair(
  baseInput: string,
  targetInput: string
) {
  const baseCurrency =
  ...
  const rate =
    ratesResult.rates[targetCurrency];
  ...
  if (
    ...
    throw new AppError(
      `No se encontró la tasa ${baseCurrency}/${targetCurrency}`,
      404
    );
  }
