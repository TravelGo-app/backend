...
export async function registerUser(data) {
    ...
    try {
        ...
        const balances = await createInitialBalances(client, wallet.id, INITIAL_BALANCES);
...
export async function loginUser(data) {
    ...
    try {
        ...
        const passwordIsValid = await comparePassword(data.password, user.password_hash);
        if (!passwordIsValid) {
            throw new AppError("Credenciales inválidas", 401);
        }
        const token = generateToken({
